...
| `td_webserver.py` | `TD_DEVICE_ACTIONS_ENABLED` | Enabled | Enables or disables all active Device Diagnostics actions. Accepted values are `1`, `true`, `yes`, `on`, `0`, `false`, `no`, and `off` (case-insensitive; blank is unset); invalid non-empty values fail startup. `--disable-device-actions` takes precedence. |
| `td_webserver.py` | `TD_DEVICE_RESET_ENABLED` | Enabled | Enables or disables destructive OTBR Reset Counters when device actions are enabled. Accepted values match `TD_DEVICE_ACTIONS_ENABLED`; invalid non-empty values fail startup. `--disable-device-reset` takes precedence. |
