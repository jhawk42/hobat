...
- For work under `plan/`, retain the active work-item ID and file across follow-ups such as "approved," "implement," and "finish." Re-read that item before acting, satisfy every acceptance criterion, and update its status before reporting it complete.
- When asked for a commit message for staged work, inspect only `git diff --cached`, follow the repository's recent commit style, and return a concise message without staging or committing files.
...
- When changing dataset registries, adaptors, field aliases, or Python/JavaScript contract mirrors, run focused tests first, then `python3 -m pytest -q tests` before declaring completion.
- For frontend changes, use the linked browser and start `td_webserver.py` on port 9178 with `--datadir ./data`. Before the first Sync, enable the hidden Cache Only control by setting `#chk-only-cache.checked = true` and dispatching a bubbling `change` event. Device Diagnostics are enabled by the normal server default; add `--disable-device-actions` only when testing the hidden state. Derive source/dataset/filter inventories from rendered DOM options, and hash `data/` before and after broad acceptance runs so tests cannot silently repair fixtures through live calls.
- After frontend edits, refresh the linked browser before testing. Exercise the exact reported interaction sequence and verify both the expected behavior and preserved surrounding behavior rather than relying on static appearance alone.
