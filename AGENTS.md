...
- For frontend changes, use the linked browser and start `td_webserver.py` on port 9178 with `--datadir ./data`. Before the first Sync, enable the hidden Cache Only control by setting `#chk-only-cache.checked = true` and dispatching a bubbling `change` event. Add `--enable-device-actions` only when testing Device Diagnostics. Derive source/dataset/filter inventories from rendered DOM options, and hash `data/` before and after broad acceptance runs so tests cannot silently repair fixtures through live calls.
