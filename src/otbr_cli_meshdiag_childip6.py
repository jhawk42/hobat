...
def fetch_meshdiag_child_ip6_for_device(parent_rloc16, router=None, extaddr_map=None):
    ...
    if table_status != "success":
        return add_meshdiag_table_evidence(build_timeout_error_record(
            ...
            rloc_key="rloc16",
        ), command="meshdiag childip6", table_status=table_status,
...
def fetch_all_meshdiag_child_ip6_tables(
    ...
    if output_path is not None:
        ...
        logging.info(
            "Saved meshdiag router childip6 tables with %d entries into %s.",
            len(results),
            output_path,
        )
    return results
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        fetch_all_meshdiag_child_ip6_tables(
            extaddr_map,
