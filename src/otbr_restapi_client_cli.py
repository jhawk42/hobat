...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--raw", action="store_true", help="Return raw API envelopes instead of flattened output")
...
def _add_node_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    node_parser = subparsers.add_parser("node", help="Read or mutate local OTBR node data")
    node_subparsers = node_parser.add_subparsers(dest="node_command", required=True)

    node_get = node_subparsers.add_parser("get", help="Get the OTBR node record from /api/node")
    ...
    dataset_parser = node_subparsers.add_parser("dataset", help="Operate on node datasets")
    dataset_subparsers = dataset_parser.add_subparsers(dest="dataset_kind", required=True)
    active_parser = dataset_subparsers.add_parser("active", help="Operate on active dataset")
    active_subparsers = active_parser.add_subparsers(dest="dataset_command", required=True)
    active_get = active_subparsers.add_parser("get", help="Get active dataset")
    active_get.add_argument("--text", action="store_true", help="Request text/plain TLV dataset")

    active_set = active_subparsers.add_parser("set", help="Create or update active dataset")
    ...
    group.add_argument("--text-file", help="Path to a text file containing the TLV dataset")


def _add_devices_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    devices_list.add_argument("--with-meta", action="store_true", help="Include collection meta with flattened items")
...
def _add_diagnostics_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    diagnostics_parser = subparsers.add_parser("diagnostics", help="Read OTBR diagnostics")
    diagnostics_subparsers = diagnostics_parser.add_subparsers(dest="diagnostics_command", required=True)

    diagnostics_list = diagnostics_subparsers.add_parser("list", help="List diagnostics")
    _add_fields_argument(diagnostics_list)
    diagnostics_list.add_argument("--with-meta", action="store_true", help="Include collection meta with flattened items")

    diagnostics_get = diagnostics_subparsers.add_parser("get", help="Get a diagnostic by diagnostics ID")
    diagnostics_get.add_argument("--diagnostics-id", required=True)


def _add_actions_commands(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    actions_parser = subparsers.add_parser("actions", help="Read or enqueue OTBR actions")
    ...
    actions_list.add_argument("--with-meta", action="store_true", help="Include collection meta with flattened items")

    actions_get = actions_subparsers.add_parser("get", help="Get an action by action ID")
    actions_get.add_argument("--action-id", required=True)

    enqueue_parser = actions_subparsers.add_parser("enqueue", help="Enqueue a new OTBR task")
    enqueue_subparsers = enqueue_parser.add_subparsers(dest="enqueue_type", required=True)

    add_thread_device = enqueue_subparsers.add_parser("add-thread-device", help="Enqueue addThreadDeviceTask")
    ...
    get_network_diag = enqueue_subparsers.add_parser("get-network-diagnostic", help="Enqueue getNetworkDiagnosticTask")
    get_network_diag.add_argument("--destination", required=True)
    get_network_diag.add_argument("--types", nargs="+", required=True, help="Diagnostic TLVs by name or integer")
    get_network_diag.add_argument("--timeout", type=int)
    get_network_diag.add_argument("--destination-type")

    reset_network_diag = enqueue_subparsers.add_parser("reset-network-diag-counter", help="Enqueue resetNetworkDiagCounterTask")
    reset_network_diag.add_argument("--types", nargs="+", required=True, help="Counter TLVs by name or integer")
    ...
    energy_scan = enqueue_subparsers.add_parser("get-energy-scan", help="Enqueue getEnergyScanTask")
    ...
    update_devices = enqueue_subparsers.add_parser("update-device-collection", help="Enqueue updateDeviceCollectionTask")
...
def dispatch(args: argparse.Namespace) -> Any:
    ...
    if args.resource == "devices":
        if args.devices_command == "list":
            return client.list_devices(fields=fields, raw=args.raw, with_meta=args.with_meta)
    ...
    if args.resource == "diagnostics":
        if args.diagnostics_command == "list":
            return client.list_diagnostics(fields=fields, raw=args.raw, with_meta=args.with_meta)
    ...
    if args.resource == "actions":
        if args.actions_command == "list":
            return client.list_actions(fields=fields, raw=args.raw, with_meta=args.with_meta)
