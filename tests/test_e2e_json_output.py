...
def test_multicast_to_json_output_has_all_fields():
    ...
    assert "route_data" in device, "route_data field missing"
    ...
    assert device["route_data"]["id_sequence"] == 180
    ...
    try:
        ...
        # Verify all new TLV fields are in the JSON output
        ...
        assert "leader_data" in json_device, "leader_data missing from JSON output"
        assert "vendor_name" in json_device, "vendor_name missing from JSON output"
        assert "vendor_model" in json_device, "vendor_model missing from JSON output"
        assert "vendor_sw_version" in json_device, "vendor_sw_version missing from JSON output"
        assert "route_data" in json_device, "route_data missing from JSON output"
        
        # Verify values in JSON
        assert json_device["eui64"] == "f434f0fffe1e1774"
        assert json_device["vendor_name"] == "Apple"
        assert json_device["vendor_model"] == "Default"
        assert json_device["vendor_sw_version"] == "Default"
        assert json_device["connectivity"]["link_quality_3"] == 13
        assert json_device["leader_data"]["partition_id"] == "0x533966c4"
        assert json_device["route_data"]["id_sequence"] == 180
        assert len(json_device["route_data"]["route_data"]) == 21  # 21 routes
        
        # Verify IPv6 addresses in JSON don't contain RouteId
        json_ipv6_addrs = json_device.get("ipv6_addrs", [])
        assert len(json_ipv6_addrs) == 4, f"Expected 4 IPv6 addresses in JSON, got {len(json_ipv6_addrs)}"
        for addr in json_ipv6_addrs:
            assert "RouteId" not in addr, f"RouteId incorrectly in JSON ipv6_addrs: {addr}"
        
        # Verify route_data is separate and has proper structure
        assert isinstance(json_device["route_data"], dict)
        assert "id_sequence" in json_device["route_data"]
        assert "route_data" in json_device["route_data"]
        assert isinstance(json_device["route_data"]["route_data"], list)
        
        # Check first route entry structure
        first_route = json_device["route_data"]["route_data"][0]
        assert "route_id" in first_route
        assert "link_quality_out" in first_route
        assert "link_quality_in" in first_route
        assert "route_cost" in first_route
        assert first_route["route_id"] == "0x03"
        ...
        print(f"✓ Route data properly structured with {len(json_device['route_data']['route_data'])} routes")
...
def test_device_with_empty_vendor_fields():
    ...
    assert device["leader_data"]["partition_id"] == "0x533966c4"
    assert device["route_data"]["id_sequence"] == 183
